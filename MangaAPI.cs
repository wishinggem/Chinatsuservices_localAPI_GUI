...
public class MangaAPI
{
    ...
    public async Task UpdateCache()
    {
        ...
        {
            proccessBar.Maximum += allMangaEntries.Count;
    ...
    public void CheckForUpdatedChapeters()
    {
        ...
        {
            LogSeperateForSQLTest(LogLevel.info, $"sending update email to {email} for {updatedMangaMapSql[email].Count} manga updates using sql.");
    ...
    public void LogSeperateForSQLTest(LogLevel lv, string message)
    {
        ...
        try
        {
            ...
            {
                File.CreateText("C:\\chinatsuservices_API\\sqlLog.txt").Close();
                File.AppendAllLines("C:\\chinatsuservices_API\\sqlLog.txt", new[] { log });
            }
        }
        catch (Exception e)
